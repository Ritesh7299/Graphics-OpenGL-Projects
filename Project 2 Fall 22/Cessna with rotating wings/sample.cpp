...
const char *WINDOWTITLE = "OpenGL / GLUT Sample -- Ritesh";
...
Display( )
{
	...
	if (CockpitView) {
		gluLookAt(0., .6, .5, 0., 5., 10., 0.f, 1.f, 0.f);
	}
	else {
		gluLookAt(-5., 2., -19., 0., 5., 10., 0.f, 1.f, 0.f);
	...
	glPushMatrix();
	glTranslatef(-10., 3., 0.);
	glRotatef((GLfloat) Time * 2 * 360, 0., 1., 0.);
	glRotatef(90., 1., 0., 0.);
	glScalef(3., 3., 3.);
	glCallList(PropellerList);
	glPopMatrix();
...
InitLists( )
{
	...
	glBegin(GL_QUADS);
	for (dz = -20.f; dz <= 20.f; dz += 4.f) {
		//Layer 1:
		glColor3f(1., abs(dz) / 20, 0.);

		glVertex3f(1.41 * dx, 0, dz + 1);
		glVertex3f(0, 1.42 * dy, dz + 1);
		glVertex3f(dx, dy, dz);
		glVertex3f(dx, -dy, dz);

		glColor3f(1., abs(dz) / 20, 1.);

		glVertex3f(-1.41 * dx, 0, dz + 1);
		glVertex3f(0, 1.41 * dy, dz + 1);
		glVertex3f(dx, dy, dz);
		glVertex3f(-dx, dy, dz);


		glColor3f(0., 1., abs(dz) / 20);
		...
		glVertex3f(1.41 * dx, 0, dz + 3);
		glVertex3f(0, 1.42 * dy, dz + 3);
		glVertex3f(dx, dy, dz + 2);
		glVertex3f(dx, -dy, dz + 2);
